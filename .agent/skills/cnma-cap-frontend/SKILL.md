...
## Option B: React Clean Architecture
...
│   │   ├── IUseCase.ts               # UseCase interface (all use cases implement this)
│   │   └── Result.ts                 # Result monad (success/failure wrapper)
│   ├── domain/                       # ★ Business rules (ZERO dependencies on other layers)
│   │   ├── entities/                 # Domain models / types
│   │   ├── repositories/            # Repository INTERFACES only
│   │   │   └── IEntityRepository.ts
│   │   └── usecases/                # ★ Business logic (one class per operation)
│   │       └── GetAllEntitiesUseCase.ts
│   ├── data/                         # ★ Data access (implements domain interfaces)
│   │   ├── repositories/            # Repository implementations
│   │   │   └── EntityRepository.ts
│   │   └── datasources/             # HTTP clients, storage adapters
│   │       └── HttpClient.ts
...
│   │   ├── hooks/                   # ★ ViewModel hooks (bridge to UseCases)
│   │   │   ├── useEntityViewModel.ts
│   │   │   └── useFLPSync.ts        # ★ FLP URL sync hook
│   │   └── styles/                  # CSS/SCSS modules
│   │       └── globals.css          # ★ CDK design tokens
│   ├── di/                           # ★ Dependency Injection container
│   │   └── container.ts             # The ONLY file that knows all implementations
│   ├── App.tsx
│   └── index.tsx
...
```
Component → useViewModel hook → UseCase → IRepository (interface) → Repository (impl) → HttpClient → API
```

**Layer rules:**
- `core/` — No imports from any other layer
- `domain/` — Imports ONLY from `core/`. Defines interfaces, never implementations
- `data/` — Imports from `core/` and `domain/` (to implement interfaces)
- `presentation/` — Imports from `core/` and `domain/`. Accesses `data/` only via `di/container`
- `di/` — The ONLY place that imports concrete classes from `data/`

### Key Patterns (8 Templates)

#### 1. IUseCase — Contract Interface
> **Template**: `templates/react/core/IUseCase.ts`

All use cases implement `IUseCase<Input, Output>` with a single `execute()` method returning `Result<Output>`.

#### 2. Result — Error Handling Monad
> **Template**: `templates/react/core/Result.ts`

Replaces try/catch spaghetti. Callers always check `result.isSuccess` before accessing `result.value`. Failures carry an error message string.

#### 3. IEntityRepository — Domain Interface
> **Template**: `templates/react/domain/repositories/IEntityRepository.ts`

Generic CRUD interface (`getAll`, `getById`, `create`, `update`, `delete`). Extend for domain-specific methods. The domain layer **never** imports implementations.

#### 4. UseCase — Business Logic
> **Template**: `templates/react/domain/usecases/GetAllEntitiesUseCase.ts`

One class per operation. Receives `IEntityRepository` via constructor. Contains business rules (filtering, validation, transformation). Returns `Result<T>`.

**Naming**: `{Verb}{Entity}UseCase` — e.g., `GetDocumentsUseCase`, `CreateOrderUseCase`

#### 5. EntityRepository — Data Implementation
> **Template**: `templates/react/data/repositories/EntityRepository.ts`

Implements `IEntityRepository`. Uses `HttpClient` for API calls. Maps API responses to domain entities.

#### 6. HttpClient — Centralized API Client
> **Template**: `templates/react/data/datasources/HttpClient.ts`

Single HTTP client shared across all repositories. Handles: base URL, auth headers, CSRF tokens (SAP-specific), error normalization. **All repositories use this — never raw fetch.**

#### 7. DI Container — Wiring
> **Template**: `templates/react/di/container.ts`

The **only** file that instantiates concrete classes. All other files import interfaces. Makes it trivial to swap implementations for testing or different environments.

#### 8. useViewModel — Presentation Hook
> **Template**: `templates/react/presentation/hooks/useEntityViewModel.ts`

Bridges UseCases to React components. Encapsulates loading state, error state, data, and actions. Components call hook methods, **never UseCases directly**.
...
### 5. UI Component (`src/presentation/components/variant/VariantManagement.tsx`)
*Requires `shadcn/ui` components: Button, Input, Popover, Dialog, Checkbox, Lucide Icons.*
> **Template**: `templates/react/presentation/components/variant/VariantManagement.tsx`
...
Every new React Clean Architecture project MUST include the CDK brand design system for visual consistency across CNMA applications.

### 8.1 Design Tokens

Design tokens are defined as CSS custom properties in `globals.css`. These are the **single source of truth** for all colors.

| Token | Value | Usage |
|-------|-------|-------|
| `--brand-red` | `#b00000` | Primary brand color, sidebar accents |
| `--accent-color` | `#b00000` | Buttons, links, active states |
| `--accent-hover` | `#8a0000` | Hover state for brand elements |
| `--bg-color` | `#f5f7fa` | Page background |
| `--card-bg` | `#ffffff` | Card/panel backgrounds |
| `--text-primary` | `#32363a` | Primary text |
| `--text-secondary` | `#6a6d70` | Secondary/muted text |
| `--border-color` | `#d9d9d9` | Borders, dividers |

Status colors: `--success-bg/text`, `--warning-bg/text`, `--error-bg/text`, `--info-bg/text`

Font stack: `"72", "72full", "Inter", "Segoe UI", Arial, sans-serif` (SAP Fiori / 72 family)

> **Template**: `templates/react/presentation/styles/globals.css`

### 8.2 MainLayout — Responsive Branded Sidebar

The `MainLayout` component provides a responsive shell with:
- **Desktop:** Red brand line at top + collapsible sidebar (w-16 icon-only ↔ w-56 expanded)
- **Mobile:** Header with hamburger button + overlay sidebar with backdrop
- **Active nav:** `bg-red-50 text-brand` | Inactive: `text-text-secondary hover:bg-slate-50`
- **Section headers:** Uppercase, tracking-wider, text-xs for grouping nav items
- **Smooth transitions:** 300ms ease-in-out

**Customization points:**
- `{{APP_TITLE}}` — Replace with project display name
- `NAV_ITEMS` array — Add/remove navigation entries with lucide icons
- Section headers — Label groupings as needed

> **Template**: `templates/react/presentation/layout/MainLayout.tsx`

### 8.3 cn() Utility

The `cn()` function merges Tailwind classes safely (handles conflicts like `p-2 p-4` → `p-4`).

```typescript
import { cn } from '@/lib/utils';

// Usage:
<div className={cn('p-4 text-sm', isActive && 'bg-red-50 text-brand')} />
```

> **Template**: `templates/react/lib/utils.ts`
...
| `tailwind-merge` | Smart Tailwind class merging (avoids conflicts) |

### 8.5 Tailwind v4 Setup (react-v2 Standard)

> ⚠️ **`@cnma/react-ui` requires Tailwind v4.** Do NOT use Tailwind v3 with react-v2 template.
...
> ⚠️ **NEVER put `@tailwind base/components/utilities` in `index.css`** — that's Tailwind v3 syntax.
> In v4, those directives are inside `@cnma/react-ui/styles.css` via `@import "tailwindcss"`.

**Root cause of `Unknown word "use strict"` error:**
Occurs when Tailwind v3 is installed alongside `@cnma/react-ui` (Tailwind v4).
`postcss-import` follows `@import` chains and hits `tailwindcss/lib/index.js` (CJS JS — not CSS).
Fix: upgrade to Tailwind v4 as shown above.

### 8.6 `@cnma/react-ui` Local File Reference

When referencing the local library package from a UI folder at `app/<ui_folder>/`:
```json
"@cnma/react-ui": "file:../../../cnma-react-ui"
```
Path depth from `app/<ui_folder>/`:
- `../` → `app/`  
- `../../` → `<project_root>/`  
- `../../../` → `cdk_design_guidelines/` ← where `cnma-react-ui/` lives

> ⚠️ The library's `dist/` folder must exist. Run `npm run build` inside `cnma-react-ui/` if missing.

### 8.7 CAP Backend Local Dev Setup
...
And in `globals.css` replace `@import "tailwindcss"` with:
```css
@tailwind base;
@tailwind components;
@tailwind utilities;
```
Remove the `@theme { ... }` block (it's Tailwind v4 only).

### 8.6 Tailwind v4 Configuration

If using Tailwind CSS v4 (`@tailwindcss/vite` plugin), tokens are registered via `@theme` block in `globals.css`:
...
@theme {
    ...
    --color-border-custom: var(--border-color);
}
```

No `tailwind.config.js` needed — Tailwind v4 reads `@theme` natively.

**Decision tree:** Check `package.json` → if `@tailwindcss/vite` exists → use v4 approach. If `tailwindcss` < 4.0 → use v3 approach.

### 8.7 Button Variants — Semantic Usage Map

The `Button` component includes semantic variants beyond shadcn defaults. **Always use semantic variants** instead of raw className overrides.

| Variant | Appearance | Use For | Example |
|---------|-----------|---------|---------|
| `default` | Brand red bg, white text | Primary actions (Add, Save, Submit, Reload) | `<Button>Add Schedule</Button>` |
| `destructive` | Red bg, white text | Irreversible destructive (Delete record) | `<Button variant="destructive">Delete</Button>` |
| `outline` | White bg, gray border | Neutral secondary (Cancel, Export, Close) | `<Button variant="outline">Export</Button>` |
| `outline-success` | White bg, green border/text | Safe actions (Start, Enable, Approve) | `<Button variant="outline-success">Start</Button>` |
| `outline-danger` | White bg, red border/text | Cautious destructive (Stop, Clear logs) | `<Button variant="outline-danger">Stop</Button>` |
| `outline-warning` | White bg, orange border/text | Warning actions (Pause, Disable) | `<Button variant="outline-warning">Pause</Button>` |
| `success` | Green bg, white text | Confirmed safe action (Start app) | `<Button variant="success">Start</Button>` |
| `ghost` | Transparent, text only | Inline/icon-only actions | `<Button variant="ghost" size="icon">` |
| `link` | Text with underline | Navigation-style | `<Button variant="link">View Details</Button>` |

**Critical:** `--primary-foreground` and `--destructive-foreground` MUST be `oklch(1 0 0)` (white) in `globals.css`. Without this, button text renders black.

> **Template**: `templates/react/src/presentation/components/ui/button.tsx`

### 8.8 Shared Components

Reusable components under `presentation/components/custom/` and `presentation/components/table/`. Import via barrel:

```typescript
import { StatusBadge, ConfirmDialog, FormDialog, LoadingSpinner } from '@/presentation/components/custom';
...
> **Template**: `templates/react/src/presentation/components/custom/StatusBadge.tsx`

#### ConfirmDialog
Modal confirmation for destructive or important actions. Supports `variant="destructive"` for red confirm button.
...
> **Template**: `templates/react/src/presentation/components/custom/ConfirmDialog.tsx`
...
> **Template**: `templates/react/src/presentation/components/custom/FormDialog.tsx`
...
> **Template**: `templates/react/src/presentation/components/custom/LoadingSpinner.tsx`

#### DataTable
TanStack Table wrapper with sorting, filtering, infinite scroll (load more on scroll). `pageSize` controls batch size for progressive rendering.

```tsx
<DataTable columns={columns} data={items} loading={isLoading} pageSize={10} />
```

> **Template**: `templates/react/src/presentation/components/table/DataTable.tsx`

### 8.9 Shared Hooks

#### useConfirmDialog
Imperative hook for confirm dialogs — avoids open/config state boilerplate per page.
...
> **Template**: `templates/react/src/presentation/hooks/useConfirmDialog.ts`
...
> **Template**: `templates/react/src/presentation/hooks/useToast.ts`

### 8.10 Filter & ComboboxFilter Patterns
...
| `disabled` | boolean | Prevents opening popover, grays out |
...
- When cascading (e.g., Credentials → Region → Org → Space), disable downstream dropdowns until parent is selected
- Auto-select first option after loading cascade data

> **Template**: `templates/react/src/presentation/components/filters/ComboboxFilter.tsx`

### 8.11 Page Layout Patterns
...
**Filter bar:** Place CfCascadeFilter or ComboboxFilter group above data content, inside a card.
