...
| `outline`     | White (border-red)     | Brand red| Cancel / secondary               |
...
- **StatusBadge** — Automatic status-to-color mapping (see §3)
...
Use these prompt templates to generate Conarum-themed screens in Google Stitch:

### List Report (Worklist)
> Generate a **List Report page** for [ENTITY_NAME] using the Conarum design system.
> - Brand color: **#b10e10** (Conarum red). SAP 72 font.
> - Top: collapsible **FilterBar** with text search, multi-select dropdowns, and date range filter. Include a "Variant Selector" dropdown at the left.
> - Main content: white Card with rounded-xl border shadow, sticky toolbar (title with count, Export/Sort/Settings buttons), and a data table with columns: [COLUMNS].
> - Rows are expandable to show detail cards in a muted background area.
> - Status badges use palette-derived colors: orange for New/Draft, blue for In Progress, green for Approved/Completed, purple for Sent/Shipped, red for Rejected.
> - Page background: `#edeff0`. Card background: `#ffffff`.

### Object Page
> Generate an **Object Page** for [ENTITY_NAME] using the Conarum design system.
> - Header Card: entity title (bold 2xl) + StatusBadge + type Badge, Edit button top-right.
> - KPI row: 4–5 compact metric cards (icon + label + value) in a 2×5 grid.
> - Tab navigation below header: Items, Details, History, Notes.
> - Items tab: DataTable with columns [COLUMNS]. Details tab: key-value form grid (2-col).
> - Brand: #b10e10. Font: SAP 72. Background: #edeff0.

### Analytical Dashboard
> Generate an **Analytical Dashboard** for [DOMAIN] using the Conarum design system.
> - FilterBar at top with date range and category filters.
> - KPI row: 4 cards showing metric (2xl bold), trend indicator (↗/↘/⚠), and change text.
> - Charts row: Pie chart (category breakdown) + Bar chart (trend over time) side by side.
> - Bottom: DataTable (top items by metric) + Alerts sidebar (warning/info with resolve actions).
> - Brand: #b10e10. Font: SAP 72. Status colors: green=positive, red=negative, orange=warning, blue=info.

### Wizard Process
> Generate a **Wizard/Stepper page** for [PROCESS_NAME] using the Conarum design system.
> - Horizontal stepper: numbered circles with progress line, brand-red active/completed states.
> - Step content area with form fields inside a borderless white Card.
> - Fixed footer: Cancel (ghost), Back (outline), Next Step / Submit (brand-red `create` button).
> - Steps: [STEP_1, STEP_2, STEP_3, ...].
> - Brand: #b10e10. Font: SAP 72.

### Flexible Column Layout
> Generate a **Flexible Column Layout** (Master-Detail) for [ENTITY_NAME] using the Conarum design system.
> - 3-column responsive layout inside a rounded-xl bordered Card.
> - Left column: master list DataTable (borderless variant).
> - Mid column: selected item detail with KPI cards, notes, and line items sub-table.
> - Right column: sub-item detail view.
> - Columns show/hide dynamically. Mobile: single column.
> - Brand: #b10e10. Font: SAP 72. Background: #edeff0.
...
### ✅ Do
- Use semantic status badges (`StatusBadge` component) — never hand-pick badge colors
- Use brand `#b10e10` for primary actions only — don't overuse
...
- Don't skip the ShellBar — every page should have a top app bar
