...
2.  **Call Skill**: `/cnma-cap-backend`
    *   Instruction: "Use the `cnma-cap-backend` skill to create the DB and SRV layers for project **{{project_name}}** with namespace **{{namespace}}**. Include the `--variant` flag if the user requested Variant Management."
    *   The skill will use the namespace in:
        - `db/schema/*.cds` → `namespace {{namespace}};`
        - `srv/*.cds` → `using {{namespace}} as ns from '../db/schema/...';`
